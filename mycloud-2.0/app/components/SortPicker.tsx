...
export const SortPicker = () => {
    ...
    return ( 
        <div className="w-[90%] flex flex-col gap-2 border-t border-gray-400 pt-3">
            <p className="text-sm uppercase font-bold text-gray-500">SORT BY</p>
            <div className="flex h-8">
                <div className={`flex items-center w-[50%] border border-stone-500 rounded-l-full p-2 gap-1 cursor-pointer ${preference === "uploadedAt" ? "bg-gray-400" : "hover:bg-blue-200"}`} onClick={() => setPreference("uploadedAt")}>
                    <Image src="./calendar-down-arrow.svg" alt="calendar-down-arrow" width={16} height={16}/>
                    <p>Upload date</p>
                </div>

                <div className={`flex items-center w-[50%] border border-l-0 border-stone-500 rounded-r-full p-2 gap-1 cursor-pointer ${preference === "name" ? "bg-gray-400" : "hover:bg-blue-200"}`} onClick={() => setPreference("name")}>
